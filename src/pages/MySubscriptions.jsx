...
const MySubscriptions=()=>{
    ...
    const subscriptions=useSelector((state)=>state.subscription?.mySubscription)
    const subscriberId=useSelector((state)=>state.auth?.loginUserData?.id)
    ...
    return (
        <>
            <div className="flex gap-2 p-2 dark:text-white items-center dark:bg-[#222222] bg-gray-300">
            ...
            <div className="dark:text-white mb-20 sm:mb-0 w-full grid xl:grid-cols-3 sm:grid-cols-2 grid-cols-1 overflow-y-scroll">
                {subscriptions?.map((subscription)=>(
                    <Link
                    ...
                    >
                        {subscription?.subscribedChannel?.latestVideo &&(
                            ...
                            channelName={subscription.subscribedChannel?.username}
                            videoId={subscription.subscribedChannel?.latestVideo?.id}
                            />
                        )}
                    </Link>
                ))}
